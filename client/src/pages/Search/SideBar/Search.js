    ...
    <div className="w-100">
      <form onChange={onChange}>
        <label htmlFor="searchSite" className="w-100">
          <input name="s" type="search" id="searchSite" placeholder="Keyword..." className="form-control p-2" />
        ...
        <label htmlFor="searchPractice" className="d-block w-100">
          <select name="practice" id="searchPractice" className="form-control p-2">
            ...
            {
              allPractices.map(p => <option value={p.title} id={addRandomKey(p.title)} key={addRandomKey(p.title)}>{p.title}</option>)
            }
          </select>
        ...
        <label htmlFor="searchAttorney" className="d-block w-100">
          <select name="attorney" id="searchAttorney" className="form-control p-2">
            ...
            {
              allAttorneys.map(p => <option value={p.title} id={addRandomKey(p.title)} key={addRandomKey(p.title)}>{p.title}</option>)
            }
          </select>
        ...
        <label htmlFor="searchCategory" className="d-block w-100">
          <select name="category" id="searchCategory" className="form-control p-2">
            ...
            {
              allCategories.map(p => ((p.name !== 'Uncategorized') ? <option value={p.name} id={addRandomKey(p.name)} key={addRandomKey(p.name)}>{p.name}</option> : ''))
            }
          </select>
