...
export default function CyberSecurityClientAlert({ post, posts }) {
...
export async function getStaticPaths() {
  const res = await client.query(getListOfPostsByName('cyber-security-client-alert'), {});

  const urlWithOutBaseUrl = res.data.posts.nodes.map((u) => {
    if (u.uri.indexOf('/cyber-security-client-alert/') < 0) {
      const uriSplit = u.uri.split('/').filter((a) => a !== '');
      const slug = uriSplit[uriSplit.length - 1];

      return `/cyber-security-client-alert/${slug}`;
    }
