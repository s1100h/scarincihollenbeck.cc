...
export default function CategoryBreadCrumbs() {
  ...
  const breadCrumbPath = currentPath.filter(
  ...
  return (
    <>
      <Link href="/">
        <a className="red-title proxima-bold h6">HOME</a>
      </Link>
        ...
        && breadCrumbPath.map((crumb, index) => (
          <span key={crumb}>
            <Link href="/category[slug]" as={`/category/${crumb}`}>
              <a className="red-title proxima-bold h6">{makeTitle(crumb)}</a>
            </Link>
            {breadCrumbPath.length - 1 !== index && (
              <strong className="text-black mt-2 mx-2 proxima-bold">
                <FontAwesomeIcon
                  icon={faCaretRight}
                  className="red-title mw-6"
                />
              </strong>
            )}
          </span>
