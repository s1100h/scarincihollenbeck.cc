...
export default function CovidAlerts({ post }) {
      ...
      {(post === undefined && router.isFallback) ? (
        ...
        <>
          <NextSeo
            ...
            canonical={`https://scarincihollenbeck.com/covid-19-alerts/${post.seo.canonicalLink}`}
            openGraph={{
            ...
            twitter={{
              handle: '@S_H_Law',
              site: `https://scarincihollenbeck.com/covid-19-alerts/${post.seo.canonicalLink}`,
...
export async function getServerSideProps({ params, res }) {
  const [post] = await Promise.all([
    fetch(`${process.env.REACT_APP_WP_BACKEND}/wp-json/single/post/${params.slug[params.slug.length - 1]}/covid-19-alerts`, { headers }).then((data) => data.json()),
  ]);
