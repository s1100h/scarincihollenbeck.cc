...
export default function QuickNewsPost({ post }) {
      ...
      {(post === undefined && router.isFallback) ? (
        ...
        <>
          <NextSeo
            ...
            canonical={`https://scarincihollenbeck.com/quick-news/${post.seo.canonicalLink}`}
            openGraph={{
              url: `https://scarincihollenbeck.com/quick-news/${post.seo.canonicalLink}`,
              title: post.seo.title,
              description: post.seo.metaDescription,
              type: 'article',
              article: {
            ...
            twitter={{
              handle: '@S_H_Law',
              site: `https://scarincihollenbeck.com/quick-news/${post.seo.canonicalLink}`,
...
export async function getServerSideProps({ params, res }) {
  const [post] = await Promise.all([
    fetch(`${process.env.REACT_APP_WP_BACKEND}/wp-json/single/post/${params.slug[params.slug.length - 1]}/quick-news`, { headers }).then((data) => data.json())
  ]);
