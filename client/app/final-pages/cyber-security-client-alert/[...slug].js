...
export default function CybserSecurityClientAlert({ slides, post }){ 

  return (
    <>
      <NavBar />
      {(post === undefined && slides === undefined) ? (
        <Container>
        ...
        <>
          <Head>
            ...
            <meta property="og:url" content={`https://scarincihollenbeck.com/cyber-security-client-alert/${post.seo.canonicalLink}`} />
            ...
            <meta property="og:image:secure_url" content={`https://scarincihollenbeck.com/cyber-security-client-alert/${post.seo.canonicalLink}`} />
            ...
            <script type="application/ld+json">
              {`
                ...
                "url": "https://scarincihollenbeck.com/cyber-security-client-alert/${post.seo.canonicalLink}",
...
export async function getStaticPaths() {
  const postsResponse = await fetch(`${process.env.REACT_APP_WP_BACKEND}/wp-json/single/list/cyber-security-client-alert`, { headers });
  const postsJson = await postsResponse.json();

  return  {
    paths: postsJson.map(link => `/cyber-security-client-alert/[...slug]/${link}`) || [],
    fallback: true,
  }
}
