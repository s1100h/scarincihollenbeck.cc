...
class Author extends Component {
  ...
  componentDidMount() {
    ...
    if (!(currentUrl.indexOf('author-index') > -1)) {
      // check for /page url
      if (currentUrl.indexOf('/author/') > -1 && currentUrl.indexOf('/page/') > -1) {
        ...
        // set breadcrumbs
        authorName = currentUrl.split('/author/').pop();
        const filtered = authorName.split('/').filter(a => a !== 'page');
        const breadCrumb = filtered;
        categorySlug = filtered[0];

        this.setState({ breadCrumb, categorySlug, currentPage: page }, () => {
          this.getPosts(`${process.env.API_URL}/wp-json/author/posts/${categorySlug}/${page}`, categorySlug);
        });
      } else {
        authorName = currentUrl.split('/author/').pop();
        authorName = authorName.replace('/', '');
        this.setState({ breadCrumb: [authorName, 1], categorySlug: authorName, currentPage: 1 }, () => {
          this.getPosts(`${process.env.API_URL}/wp-json/author/posts/${authorName}/1`, authorName);
        });
      }
    } else {
      authorName = 'fbrunetti';
      page = 1;
      this.setState({ currentPage: page, categorySlug: authorName, breadCrumb: [authorName] }, () => {
        this.getPosts(`${process.env.API_URL}/wp-json/author/posts/${authorName}/${page}`, authorName);
      });
    }
  }
        ...
        <div className="row">
          <div className="col-sm-12 mt-0 mb-4">
            <h6>
              ...
              { (breadCrumb.length > 0) ? breadCrumb.map((val, indx) => ((indx < breadCrumb.length - 1) ? (
                <span key={val}>
                  <span>
                  ...
                  ? (
                    <nav aria-label="pagination">
                      <ul className="d-flex flex-wrap no-dots lead">
                        <li className="mr-2">
                          <a className="text-dark" href={`${window.location.origin}/author/${term}/page/${prev}/`} tabIndex="-1" aria-label="previous link">
                        ...
                        {/** Current / First Number */}
                        {
                          pageNums.map(val => ((active === val) ? (
                            <li className={(active === val) ? 'active mr-2' : 'mr-2'} key={`page-${val}`}>
                              <a className="text-dark mt-2" href={`${window.location.origin}/author/${term}/page/${val}/`}>{val}</a>
                            </li>
                          ) : ''))
                        }
                        {/** ... */}
                        ...
                        {/** Last Number */}
                        {
                          pageNums.map(val => (((pageNums.length - 1) === val) ? (
                            <li className={(active === val) ? 'active mr-2' : 'mr-2'} key={`page-${val}`}>
                              <a className="text-dark" href={`${window.location.origin}/author/${term}/page/${val}/`}>{val}</a>
                            </li>
                          ) : ''))
                        }
                        <li className="ml-1">
                          <a className="text-dark" href={`${window.location.origin}/author/${term}/page/${next}/`} aria-label="next link">
