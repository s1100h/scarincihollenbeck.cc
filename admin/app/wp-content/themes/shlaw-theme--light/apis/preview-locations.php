...
function sort_obj_results_prev($data)
{
	$results = array();
	if(count($data) > 1) {
		foreach($data as $d) {
...
function sort_r_links_prev($data)
{
	$results = array();
	if(count($data) > 1) {
		foreach($data as $d) {
...
function preview_location($request)
{
  $id = $request["id"];
  ...
  // get the office post slug
  $slug =  str_replace("-"," ", $office[0]->post_name);
  $slug = ucwords($office[0]->post_name);
  ...
  $practice_data = array();
  $practices = get_field("office_practices", $office_id);
  
  if($practices){
    $practice_data = sort_r_links_prev($practices);
  }
  ...
  $args = array(
    ...
    'meta_query' => array(
      array(
          ...
          'value' => '"'.$office_id.'"', // matches exactly "red"
          'compare' => 'LIKE'
      )
    )
  );
  ...
  if($related_attorneys) {
    $attorney_data = sort_obj_results_prev($related_attorneys);
  }
  
  // combine address fields into a single string 
  $building_title = get_field("office_building_title", $office_id);
  $street_address = get_field("street_address", $office_id);
  $po_box = get_field("po_box", $office_id);
  $floor = get_field("floor", $office_id);
  $address_locality = get_field("address_locality", $office_id);
  $address_region = get_field("address_region", $office_id);
  $post_code = get_field("post_code", $office_id);
  $address_country = get_field("address_country", $office_id);
  ...
  $office_data = array(
    "id" => $office_id,
    "name" => html_entity_decode(htmlspecialchars_decode(get_the_title($office_id))),
    "address" => $address_array,
    "phone" => get_field("phone", $office_id),
    "fax" => get_field("fax", $office_id),
    "mapLink" => get_field("map_link", $office_id),
    "practices" => $practice_data,
    "attorneys" => $attorney_data,
    "seo" => (object)array(
      "title" => get_post_meta( $office_id, '_yoast_wpseo_title', true),
      "metaDescription" => get_post_meta( $office_id, '_yoast_wpseo_metadesc', true),
      ...
      "latitude"=>  get_field("latitude", $office_id),
      "longitude"=>  get_field("longitude", $office_id),
      "url"=>  "location/".html_entity_decode(htmlspecialchars_decode(get_the_title($office_id))),
      "telephone"=>  get_field("phone", $office_id),
      "image" => get_the_post_thumbnail_url($office_id, 'full')
    )
  );
  return $office_data;
}
 ...
 */
function preview_location_posts($request) {
  $slug = $request['slug'];
