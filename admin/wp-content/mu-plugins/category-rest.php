...
function get_attorney_info($authors) {
  ...
  foreach($authors_email as $ae) {
    ...
    if($ae === "info@sh-law.com") {
      ...
      $authors_data[] = array(
        "t" => $ae,
        "name" => $apost[0]->post_title,
        "link" => (get_the_author_meta("email", $author->ID) === "info@sh-law.com") ? get_home_url("/") : get_home_url("/")."/attorneys/".$apost[0]->post_name
      );
    }
  }
...
function category_data($request) {
  ...
  $post_data['main'][] = array (
    ...
    "link" => get_the_permalink($posts[0]->ID),
    //
    "image" =>  get_featured_image($posts[0]->post_content),
    "category" => array (
      "name" => get_the_category_by_ID(wp_get_post_categories($posts[0]->ID)[0]),
      "link" => get_home_url("/")."/category/".get_category(wp_get_post_categories($posts[0]->ID)[0])->slug
    ),
  ...
  if(count($terms) > 0) {
    foreach($terms as $t) {
      $post_data['practices'][] = array(
        "id" => $t->term_id,
        "name" => $t->name,
        "link" => get_home_url("/")."/category/".$slug."/".$t->slug,
  ...
  if($current_category->parent !== 0){
    $post_data['parent'] = array(
      ...
      "link" => get_home_url("/")."/category/".get_category($current_category->parent)->slug
    );
  }else {
    $post_data['parent'] = array();
  }
  ...
  foreach($author_ids as $ai) {
    ...
    if($web_url !== "") {
      $post_data['authors'][] = array(
        ...
        "link" => get_home_url("/").'/author/'.get_the_author_meta('user_nicename', $ai),
        "id" => $ai,
