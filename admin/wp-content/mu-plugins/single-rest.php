...
function single_data($request) {
  ...
  foreach($authors as $a) {
    ...
    $authors_data[] = array(
      ...
      "link" => (get_the_author_meta("email", $a->ID) === "info@sh-law.com" || get_the_author_meta("email", $a->ID) === "scarincihollenbeckmarketing@gmail.com") ? get_home_url("/") : $a->user_url,
  ...
  foreach($category_list as $cl) {
    ...
    $categories[] = array(
      "id" => $data->term_id,
      "title" => $data->name,
      "link" => get_home_url('/').'/category/'.$data->slug
    );
  }
