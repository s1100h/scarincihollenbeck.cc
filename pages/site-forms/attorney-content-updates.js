      ...
      <Row>
        ...
        >
          <label htmlFor="attorney-selected">
            <h3><strong>1. Please select your name?</strong></h3>
          ...
          <div>
            <label htmlFor="written-in-attorney-name">
              <h3><strong> Not listed? Please write in your name</strong></h3>
              <input
                type="text"
          ...
          <>
            ...
            <p className="mb-4">
              ...
              {attorneys.filter((a) => a.name === attorney).map((b) => <a key={b.link} href={b.link} target="_blank" rel="noopenner noreferrer">{b.link}</a>)}
            </p>
          </>
          )}
...
export async function getStaticProps() {
  ...
  return {
    props: {
      ...
      practices: practiceRequest.map((practice) => practice.title.rendered.replace(/&#038;/g, '&').replace(/&#8217;/g, "'")).sort((a, b) => {
        if (a > b) return 1;
        return -1;
      }),
    },
